...
function Dashboard() {
  ...
  const Username = user ? JSON.parse(user).username : ''; // Mengambil username dari localStorage
  const Inisial =  Username.charAt(0).toUpperCase(); // Mengambil huruf pertama dari username dan mengubahnya menjadi huruf kapital
  // simpan link ke yag ditambhkan ke localStorage
  const handleAddLink = () => {
    if (title && link) {
        ...
        <div className="container">
          <div className="container">
            <div className="col d-flex justify-content-center my-5">
              <h1 className="mx-1 profile-inmain">{Inisial}</h1>
              <div>
                <span>@{Username}</span>
                <br />
                <small>Add bio</small>
